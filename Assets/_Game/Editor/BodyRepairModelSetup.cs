...
{
    /// <summary>Installs the authored Body Repair model without replacing station gameplay.</summary>
    public static class BodyRepairModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/body_repair_final_1k.glb";
        private const string ModelRootName = "BodyRepairModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Body Repair Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Body Repair GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_BODY_REPAIR");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_BODY_REPAIR was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Body Repair model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Body Repair model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "body_repair_1k_real" || name == "body_repair_final_1k" ||
                    name == "Shell" || name == "Sign_BODY_REPAIR" ||
