...
{
    /// <summary>Installs the authored Car Wash model without replacing station gameplay.</summary>
    public static class CarWashModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/car_wash_1k_real.glb";
        private const string ModelRootName = "CarWashModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Car Wash Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Car Wash GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_CAR_WASH");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_CAR_WASH was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Car Wash model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Car Wash model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "car_wash_1k_real" ||
                    name == "Shell" || name == "Sign_CAR_WASH" ||
