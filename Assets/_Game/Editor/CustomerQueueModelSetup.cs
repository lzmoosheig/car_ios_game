...
{
    /// <summary>Installs the authored Customer Queue model without replacing station gameplay.</summary>
    public static class CustomerQueueModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/customer_queue_1k_real.glb";
        private const string ModelRootName = "CustomerQueueModel";
        private const float TargetWidth = 10.5f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Customer Queue Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Customer Queue GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_CUSTOMER_QUEUE");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_CUSTOMER_QUEUE was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Customer Queue model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Customer Queue model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "Sign_CUSTOMER_QUEUE" ||
                    name == "ParkedCar" || name == "construction-cone" || name == "SliceDressing")
