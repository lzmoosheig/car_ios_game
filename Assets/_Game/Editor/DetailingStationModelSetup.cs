...
{
    /// <summary>Installs the authored Detailing Station model without replacing station gameplay.</summary>
    public static class DetailingStationModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/detailing_station_1k.glb";
        private const string ModelRootName = "DetailingStationModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Detailing Station Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Detailing Station GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_DETAILING_STATION");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_DETAILING_STATION was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Detailing Station model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Detailing Station model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "detailing_station_1k" ||
                    name == "Shell" || name == "Sign_DETAILING_STATION" ||
