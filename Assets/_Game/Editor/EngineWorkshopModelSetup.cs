...
{
    /// <summary>Installs the authored Engine Workshop model without replacing station gameplay.</summary>
    public static class EngineWorkshopModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/engine_workshop_1k_real.glb";
        private const string ModelRootName = "EngineWorkshopModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Engine Workshop Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Engine Workshop GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_ENGINE_WORKSHOP");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_ENGINE_WORKSHOP was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Engine Workshop model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Engine Workshop model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "engine_workshop_1k_real" ||
                    name == "Shell" || name == "Sign_ENGINE_WORKSHOP" ||
