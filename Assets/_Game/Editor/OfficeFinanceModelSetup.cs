...
{
    /// <summary>Installs the authored Office &amp; Finance model without replacing station gameplay.</summary>
    public static class OfficeFinanceModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/office_finance_1k_real.glb";
        private const string ModelRootName = "OfficeFinanceModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Office Finance Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Office & Finance GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_OFFICE_&_FINANCE");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_OFFICE_&_FINANCE was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Office & Finance model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Office & Finance model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "office_finance_1k_real" ||
                    name == "Shell" || name == "Sign_OFFICE_&_FINANCE" ||
