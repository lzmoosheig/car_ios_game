...
{
    /// <summary>Installs the authored Reception model without replacing station gameplay.</summary>
    public static class ReceptionModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/reception_1k_real.glb";
        private const string ModelRootName = "ReceptionModel";
        private const float TargetWidth = 7f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Reception Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Reception GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_RECEPTION");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_RECEPTION was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Reception model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Reception model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "Reception_1k_Real" ||
                    name == "Shell" || name == "Sign_RECEPTION" || name == "SliceDressing")
