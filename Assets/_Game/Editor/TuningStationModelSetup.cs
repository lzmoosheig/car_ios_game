...
{
    /// <summary>Installs the authored Tuning Station model without replacing station gameplay.</summary>
    public static class TuningStationModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/tuning_station_1k_real.glb";
        private const string ModelRootName = "TuningStationModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Tuning Station Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Tuning Station GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_TUNING_STATION");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_TUNING_STATION was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Tuning Station model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Tuning Station model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "tuning_station_1k_real" ||
                    name == "Shell" || name == "Sign_TUNING_STATION" ||
