...
{
    /// <summary>Installs the authored Used Car Showroom model without replacing station gameplay.</summary>
    public static class UsedCarShowroomModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/used_car_showroom_1k_real.glb";
        private const string ModelRootName = "UsedCarShowroomModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Used Car Showroom Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Used Car Showroom GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_USED_CAR_SHOWROOM");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_USED_CAR_SHOWROOM was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Used Car Showroom model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Used Car Showroom model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "used_car_showroom_1k_real" ||
                    name == "Shell" || name == "Sign_USED_CAR_SHOWROOM" ||
