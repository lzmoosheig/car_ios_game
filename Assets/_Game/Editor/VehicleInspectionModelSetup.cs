...
{
    /// <summary>Installs the authored Vehicle Inspection model without replacing station gameplay.</summary>
    public static class VehicleInspectionModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/vehicle_inspection_1k_real.glb";
        private const string ModelRootName = "VehicleInspectionModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Vehicle Inspection Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Vehicle Inspection GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_VEHICLE_INSPECTION");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_VEHICLE_INSPECTION was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Vehicle Inspection model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Vehicle Inspection model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "vehicle_inspection_1k_real" ||
                    name == "Shell" || name == "Sign_VEHICLE_INSPECTION" ||
