...
{
    /// <summary>Installs the authored Wheel & Tire Station model without replacing station gameplay.</summary>
    public static class WheelTireStationModelSetup
    {
        private const string ScenePath = "Assets/_Game/Scenes/CityGarage.unity";
        private const string ModelPath = "Assets/_Game/Art/Models/Buildings/wheel_tire_station_1k_real.glb";
        private const string ModelRootName = "WheelTireStationModel";
        private const float TargetWidth = 7.4f;
        private const float GroundOffset = 0.1f;

        [MenuItem("Overhaul/Apply Wheel & Tire Station Model")]
        public static void Apply()
        {
            ...
            Debug.Log("[Overhaul] Wheel & Tire Station GLB model applied.");
        }

        public static bool ApplyToOpenScene()
        {
            var station = GameObject.Find("Station_WHEEL_&_TIRE_STATION");
            if (station == null)
            {
                Debug.LogWarning("[Overhaul] Station_WHEEL_&_TIRE_STATION was not found.");
                return false;
            }
            ...
            if (modelAsset == null)
            {
                Debug.LogWarning($"[Overhaul] Wheel & Tire Station model is missing: {ModelPath}");
                return false;
            }
            ...
            if (instance == null)
            {
                Debug.LogWarning("[Overhaul] Could not instantiate the Wheel & Tire Station model.");
                return false;
            }
        ...
        private static void RemoveGeneratedVisuals(Transform station)
        {
            for (var i = station.childCount - 1; i >= 0; i--)
            {
                ...
                var name = child.name;
                if (name == ModelRootName || name == "wheel_tire_station_1k_real" ||
                    name == "Shell" || name == "Sign_WHEEL_&_TIRE_STATION" ||
